* Goal: Explore alternatives to enrollment ID matching since we cannot do that for the sellers.
...
merge 1:1 enrollmentid using "${file_path_owner}/oct_enrollment.dta", keep(master matched) ///
		keepusing(affiliationentityid affiliationentityname ///
					organizationname doingbusinessasname nursinghomeprovidername)
...
sort owner num_acq date large_group_id
...
merge 1:m associateid using `chow_big_gp', keep(using matched)
tab _merge owner
sum num_gp if owner == "seller" & _merge == 3, d 
count if !mi(large_group_id) & owner == "seller"
